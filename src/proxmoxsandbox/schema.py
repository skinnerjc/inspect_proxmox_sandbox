...
class SdnConfig(BaseModel, frozen=True):
    ...
    Attributes:
        ...
        use_pve_ipam_dnsnmasq: Whether to use Proxmox VE's built-in IPAM and DNSmasq
            Set to False if you are using e.g. your own pfsense instance for IPAM (recommended)
    """
...
class VmSourceConfig(BaseModel, frozen=True):
    ...
    Attributes:
        ...
        built_in: Use this provider's built-in VM template (currently "ubuntu24.04" is supported)
    """
...
class VmConfig(BaseModel, frozen=True):
    ...
    Attributes:
        ...
        is_sandbox: if True, the VM will show up as a sandbox. It must have the qemu-guest-agent installed
        uefi_boot: if True, the VM will boot in UEFI mode. In theory, this is already specified by OVA, but Proxmox doesn't seem to respect it.
...
class ProxmoxSandboxEnvironmentConfig(BaseModel, frozen=True):
    ...
    Attributes:
        ...
        user: The username for Proxmox authentication, 'root' unless you have configured custom auth
